...
public class PetEquipHud {
    public void render(DrawContext drawContext, MinecraftClient client) {
        ...
        List<Text> textList = HudFont.recolorAll(PetEquipHudHandler.instance().assemblePetText());
        ...
        try {
            ...
            textList.forEach(text -> drawContext.drawText(textRenderer, text,
                    ...
                    scaledY + (count.getAndIncrement() * lineHeight) + padding - finalHeightClampTranslation, 0xFFFFFFFF,
                    true));
        } finally {
