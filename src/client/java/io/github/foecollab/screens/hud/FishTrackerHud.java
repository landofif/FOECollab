...
public class FishTrackerHud {
    public void render(DrawContext drawContext, MinecraftClient client) {
        ...
        // Assemble all text lines (cached + recolored in the handler, not per frame)
        ...
        try {
            ...
            HudAlignment alignment = config.fishTracker.alignment;
            boolean center = alignment == HudAlignment.CENTER;
            boolean left = alignment == HudAlignment.LEFT;

            // hudX is the anchor's position measured from the screen's left edge for every
            // alignment; alignment only decides which point of the box (left edge / centre / right
            // edge) sits there, so changing alignment keeps the HUD in place instead of jumping.
            float xPercent = config.fishTracker.hudX / 100f;
            ...
            if(config.theme.themeType != Theming.ThemeType.OFF) {
                ...
                if (!config.fishTracker.hideTitle) {
