...
public class QuestHud {
    public void render(DrawContext drawContext, MinecraftClient client) {
        ...
        List<Text> textList = HudFont.recolorAll(QuestTrackerHudHandler.instance().assembleQuestText());
        ...
        try {
            if(QuestHandler.instance().isQuestInitialized()) {
                ...
                boolean rightAlignment = config.questTracker.rightAlignment;

                // Convert percentage config values to screen coordinates
                float xPercent = rightAlignment ?  1f - (config.questTracker.hudX / 100f) : config.questTracker.hudX / 100f;
                float yPercent = config.questTracker.hudY / 100f;
                ...
                // Scaling setup
                int fontSize = config.questTracker.fontSize;
                float scale = fontSize / 10.0f;
                drawContext.getMatrices().scale(scale, scale);

                // Alpha
                int alphaInt = (int) ((config.questTracker.backgroundOpacity / 100f) * 255f) << 24;
                ...
                if(config.theme.themeType != Theming.ThemeType.OFF) {
                    ...
                    if (!config.questTracker.hideTitle) {
                        Text title = QuestTrackerHudHandler.instance().getTitle().copy().withColor(ThemingHandler.instance().currentThemeType.TEXT_COLOR);
