...
public class ContestHandler {
            ...
            try {
                ...
                if (endIndex > startIndex && startIndex >= 0 && endIndex < messageText.length()) {
                    ...
                    if ((timeStr.contains("m") || timeStr.contains("s")) && !timeStr.equals("ENDED")) {                      
                        ...
                        FOECollab.LOGGER.info("[FoE] Time string does not contain valid time format: '{}'", timeStr);
                    }
                } else {
                    FOECollab.LOGGER.warn("[FoE] Invalid indices - start: {}, end: {}, length: {}", startIndex, endIndex, messageText.length());
                }
            } catch (StringIndexOutOfBoundsException e) {
                FOECollab.LOGGER.error("[FoE] StringIndexOutOfBoundsException parsing time from contest message: '{}'", messageText, e);
            } catch (Exception e) {
                FOECollab.LOGGER.error("[FoE] Unexpected error parsing time from contest message: '{}'", messageText, e);
            }
        } else {
            FOECollab.LOGGER.debug("[FoE] Message does not contain 'FISHING CONTEST RANKINGS (': '{}'", messageText);
        }
        return null;
    }
    ...
    public boolean onReceiveMessage(Text message) {
        ...
        if (messageText.contains("FISHING CONTEST RANKINGS")) {
            ...
            if (FOEConfig.getConfig().contestTracker.suppressServerMessages) {
                ...
                if (messageText.contains("FISHING CONTEST RANKINGS (ENDED)")) {
                    FOECollab.LOGGER.info("[FoE] Contest ended");
        ...
        if (messageText.startsWith("You → ")) {
            ...
            if (messageText.contains("(out of ")) {
                try {
                    ...
                    if (endIndex > startIndex) {
                        ...
                        FOECollab.LOGGER.info("[FoE] Parsed total participants: {}", this.totalParticipants);
                    }
                } catch (NumberFormatException e) {
                    FOECollab.LOGGER.warn("[FoE] Failed to parse total participants from: {}", messageText);
                }
            }
            ...
            if (!this.rankStat.isEmpty() && !this.rank.contains("Unranked")) {
                try {
                    ...
                    FOECollab.LOGGER.info("[FoE] Parsed rank stat: {} -> {} lbs", this.rankStat, this.biggestFish);
                } catch (NumberFormatException e) {
                    FOECollab.LOGGER.warn("[FoE] Failed to parse rank stat: {}", this.rankStat);
                }
            }
            ...
            try {
                if (levelText.contains("-")) {
                    ...
                    if (parts.length == 2) {
                        ...
                        FOECollab.LOGGER.info("[FoE] Parsed level range: {} -> {} to {}", levelText, this.levelLow, this.levelHigh);
                    }
                }
            } catch (NumberFormatException e) {
                FOECollab.LOGGER.warn("[FoE] Failed to parse level range: {}", levelText);
            }
        }
        if (messageText.startsWith("\uF060")) {
            ...
            FOECollab.LOGGER.info("[FoE] Parsed first place: {} → {}", this.firstName, this.firstStat);
        }
        if (messageText.startsWith("\uF061")) {
            ...
            FOECollab.LOGGER.info("[FoE] Parsed second place: {} → {}", this.secondName, this.secondStat);
        }
        if (messageText.startsWith("\uF062")) {
            ...
            FOECollab.LOGGER.info("[FoE] Parsed third place: {} → {}", this.thirdName, this.thirdStat);
        }
